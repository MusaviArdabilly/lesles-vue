<template>
  <div class="space-y-6">
    ...
    <div class="rounded-lg bg-white space-y-4 p-4">
      <div v-if="classData" class="text-center">
        <h1 class="text-xl font-bold text-gray-500 mb-1">Edit Kelas</h1>
        <p class="text-xs text-gray-600">{{ classData.name }}</p>
        <p class="text-xs text-gray-600 mb-3">
          {{ classData.members[0].name }}
          <span v-if="classData.members.length > 1">, dkk</span>
        </p>
      </div>
      <div class="space-y-3">
        <!-- Tipe Kelas  -->
        ...
        <div class="flex justify-between gap-2">
          ...
          >
            {{ isLoading ? 'Memproses...' : 'Ubah Kelas' }}
          </button>
        </div>
      </div>
    </div>
...
const classData = ref(null); // ✅ define classData
...
const isEditMode = computed(() => !!route.params.id); // ✅ check mode
...
const loadClassData = async () => {
  ...
  try {
    ...
    // Set value langsung
    form.type = c.type;
    form.subject_id = c.subject_id;
    form.name = c.name;
    form.members = c.members || [];
    form.location_address = c.location?.address || '';
    form.description = c.description || '';
    form.note = c.note || '';
    form.status = c.status || 'pending';
    form.education_level_id = c.education_level_id;

    // ✳️ Urutan penting (top-down)
    form.location_province_id = c.location?.province?.id || '';
    await loadCities(); // ✅ force-load cities
    form.location_city_id = c.location?.city?.id || '';
    await loadDistricts(); // ✅ force-load districts
    form.location_district_id = c.location?.district?.id || '';
    await loadVillages(); // ✅ force-load villages
    form.location_village_id = c.location?.village?.id || '';

    // Jadwal
    form.schedule = Array.isArray(c.schedules)
      ? c.schedules.map((item) => ({
          day: item.day || '',
...
const classSubmission = async () => {
  ...
  const payload = {
    ...
    members: form.members.map((item) => item.email),
  ...
  try {
    if (isEditMode.value) {
      const classId = route.params.id;
      await axios.put(`/operator/class/${classId}/edit`, payload);
    ...
    if (error.response?.status === 422) {
      ...
      generalError.value = error.message || 'Terjadi kesalahan server';
    }
    console.error(error);
  } finally {
    isLoading.value = false;
...
onMounted(async () => {
  ...
  if (isEditMode.value) await loadClassData();
});
</script>
