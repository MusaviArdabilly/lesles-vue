<template>
  <div class="space-y-6">
    <div class="sticky top-0">
      <div class="flex justify-between items-center">
        ...
        <div v-if="auth.user.role === 'murid'">
          <RouterLink
            to="/class/create"
            class="text-sm text-white font-medium rounded-lg bg-cyan-400 px-2 py-1 cursor-pointer hover:bg-cyan-500"
          >
            Buat Kelas
          </RouterLink>
        </div>
    ...
    <div class="mt-20">
      ...
      <div
        v-if="loading"
        class="rounded-lg bg-white border border-gray-300 p-4 animate-pulse space-y-3 animate-pulse"
      >
        <div
      ...
      <div v-else-if="classes.length" class="grid grid-cols-1 gap-4">
        ...
        >
          ...
          >
            ...
            >
              <span
                class="text-xs text-gray-600 font-medium capitalize tracking-tight bg-gray-100 rounded-lg px-2 py-1"
                >{{ item.type }}</span
              >
              <span
                class="text-xs text-green-500 bg-green-100 font-medium capitalize tracking-tight rounded-lg px-2 py-1"
                >{{ item.status }}</span
              >
            </div>
            <div class="space-y-2 mt-4">
