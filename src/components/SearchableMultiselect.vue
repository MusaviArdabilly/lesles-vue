<template>
  <div class="relative text-sm" ref="wrapper">
    ...
    <div
      ...
      class="flex flex-wrap items-center gap-2 rounded border border-gray-300 px-3 py-2 cursor-pointer"
    >
      <template v-if="modelValue.length">
        <span
          v-for="(option, index) in modelValue"
          :key="index"
          class="flex items-center rounded border border-gray-300 text-sm px-2 py-1"
        >
          {{ option[labelKey] }}
          <button
            @click.stop="removeOption(option)"
            class="border-l border-gray-300 pl-2 ml-2"
          >
            &times;
          </button>
        </span>
      </template>
      <span v-else class="text-gray-400">Pilih</span>
    </div>
